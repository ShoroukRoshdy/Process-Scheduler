...
int main(int argc, char * argv[])
{
   // Reading Algo , Quantum from argument list
    ...
    createAllPossibleMemories(memoryArray);
   ...
   if (Algo == 1)
   {
      struct Queue* Finishqueue = HPF(NumberOfProcesses,memoryArray);
      fclose(outputFile);
      fclose(memoryFile);
   ...
   {
     struct Queue* Finishqueue =  SRTN(NumberOfProcesses,memoryArray);
      fclose(outputFile);
      fclose(memoryFile);
      int finishTime = getClk();
      double avgWTA =0,Avgwait =0,Std =0,std_sum =0;
   ...
   {
     struct Queue* Finishqueue =  RR(NumberOfProcesses,Quantum,memoryArray);
      fclose(outputFile);
      fclose(memoryFile);
